...
const _PropertyGraph{T} = ACSet{
  SchemaType(TheoryPropertyGraph)...,
  Tuple{Dict{Symbol,T}},
  (:src, :tgt),
  (),
}
...
PropertyGraph{T}(; kw...) where {T} =
  PropertyGraph{T,_PropertyGraph{T}}(; kw...)
...
const _AbstractSymmetricPropertyGraph{T} = AbstractACSet{
  SchemaType(TheorySymmetricPropertyGraph)...,
  Tuple{Dict{Symbol,T}},
}
...
SymmetricPropertyGraph{T,G}(;
  kw...,
) where {T,G<:_AbstractSymmetricPropertyGraph{T}} =
...
add_vertices!(g::AbstractPropertyGraph{T}, n::Int) where {T} =
  add_parts!(g.graph, :V, n, vprops=[Dict{Symbol,T}() for i in 1:n])
...
add_edge!(
  ...
  tgt::Int,
  d::Dict{Symbol,T},
) where {T} = add_part!(g.graph, :E, src=src, tgt=tgt, eprops=d)
...
function add_edges!(
  ...
  if isnothing(eprops)
    eprops = [Dict{Symbol,T}() for i in 1:n]
  end
...
add_edge!(
  g::SymmetricPropertyGraph{T},
  src::Int,
  tgt::Int,
  d::Dict{Symbol,T},
) where {T} = add_edges!(g, src:src, tgt:tgt, [d])
...
function SymmetricPropertyGraph{T}(
  g::SymmetricGraph,
  make_vprops,
  make_eprops;
  gprops...,
