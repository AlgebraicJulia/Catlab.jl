...
function layout_to_context(diagram::WiringDiagram, opts::ComposeOptions)
  ...
  C.compose(
    C.context(units=units, tag=:diagram),
    C.compose(C.context(tag=:boxes), map(boxes(diagram)) do box
...
function layout_to_context(box::Box, opts::ComposeOptions)
  C.compose(
    C.context(lower_corner(box)..., size(box)..., units=C.UnitBox(), tag=:box),
    opts.box_renderer(box.value, opts),
  )
end

function layout_to_context(
...
function labeled_box(
  ...
  labeled_form(
    form,
    box_label(MIME("text/plain"), layout.value),
    form_props=box_props(layout, opts),
    text_props=opts.props[:text],
  )
end
...
"""
function labeled_form(
  form::C.ComposeNode,
  label::String;
  rounded::Bool=true,
  form_props::ComposeProperties=[],
  text_props::ComposeProperties=[],
)
  C.compose(
    C.context(),
...
function piecewise_curve(points::Vector{<:Tuple})
  ...
  C.compose(
    C.context(),
    (
      C.curve(points[i:(i + 3)]...) for
      i in range(1, length(points) - 1, step=3)
    )...,
  )
end
