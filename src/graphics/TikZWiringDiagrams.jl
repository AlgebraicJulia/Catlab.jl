...
function layout_to_tikz(
  ...
  props = [
    ...
    [
      TikZ.Property("$name/.style", TikZ.as_properties(props)) for
      (name, props) in styles
    ]
  ]
  TikZ.Document(
...
function tikz_box(diagram::WiringDiagram, vpath::Vector{Int}, opts::TikZOptions)
  ...
  TikZ.Statement[
    ...
    reduce(
      vcat,
      [tikz_box(box(diagram, v), [vpath; v], opts) for v in box_ids(diagram)],
      init=[],
    )
...
function tikz_port(diagram::WiringDiagram, port::Port, opts::TikZOptions)
  ...
  anchor, (x, y) =
    if shape in (:rectangle, :triangle, :invtriangle, :trapezium, :invtrapezium)
...
function tikz_styles(opts::TikZOptions)
  # Box style options.
  styles = OrderedDict(
    style => get(opts.styles, style) do
      tikz_node_style(opts, style)
    end for
    style in sort!([["outer box", "wire"]; collect(opts.used_node_styles)])
  )
  ...
  if opts.labels
    styles["wire"] = vcat(
      styles["wire"],
      tikz_decorate_markings(["at position $(opts.labels_pos) with {#1}"]),
    )
...
function tikz_node_style(opts::TikZOptions, name::String)
  ...
  @match name begin
    "outer box" => [TikZ.Property("draw", "none")]
    "wire" => [TikZ.Property("draw")]
    "box" => [
    ...
    "circular box" =>
      [TikZ.Property("circle"), TikZ.Property("draw"), TikZ.Property("solid")]
    "elliptical box" =>
      [TikZ.Property("ellipse"), TikZ.Property("draw"), TikZ.Property("solid")]
    "triangular box" => [
    ...
    "invisible" =>
      [TikZ.Property("draw", "none"), TikZ.Property("inner sep", "0")]
    _ => TikZ.Property[]
  end
end
