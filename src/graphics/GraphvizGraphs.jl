...
function parse_graphviz(doc::AbstractDict)::AbstractPropertyGraph
  ...
  for node in doc["objects"][(nsubgraphs + 1):end]
    ...
    props[:size] =
      72 *
      SVector(parse(Float64, node["width"]), parse(Float64, node["height"]))
...
function parse_spline(spline::AbstractString)
  ...
  for s in split(spline, " ")
    if startswith(s, "s,")
      start = parse_point(s[3:end])
    elseif startswith(s, "e,")
      stop = parse_point(s[3:end])
    else
      push!(points, parse_point(s))
    end
  end
...
function to_graphviz(
  ...
  for e in edges(g)
    ...
    if show_reflexive || !is_reflexive
      ...
      if is_reflexive
        set_eprop!(pg, e′, :style, "dashed")
      end
      if edge_labels
        set_eprop!(pg, e′, :label, string(e))
      end
    end
  end
  to_graphviz(pg)
...
function to_graphviz(
  ...
  for e in edges(g)
    ...
    elseif !is_reflexive && e <= inv(g, e)
      e′ = add_edge!(pg, src(g, e), tgt(g, e))
      if edge_labels
        set_eprop!(pg, e′, :label, "($e,$(inv(g,e)))")
      end
    end
  end
  to_graphviz(pg)
