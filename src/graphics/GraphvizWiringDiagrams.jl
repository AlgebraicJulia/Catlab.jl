...
const default_directed_graphviz_attrs = (
  graph=Graphviz.Attributes(:fontname => default_graphviz_font),
  ...
  cell=Graphviz.Attributes(:border => "1", :cellpadding => "4"),
)
...
function to_graphviz(
  ...
  update_port_map! =
    (v::Int, kind::PortKind, node_ids) -> begin
  ...
  if outer_ports
    gv_box =
      graphviz_outer_box(f; anchor=anchor_outer_ports, orientation=orientation)
  ...
  Graphviz.Digraph(
    ...
    graph_attrs=merge(
      default_attrs.graph,
      Graphviz.as_attributes(graph_attrs),
      Graphviz.Attributes(:rankdir => rank_dir(orientation)),
    ),
...
"""
function box_html_label(
  nin::Int,
  nout::Int,
  text_label::String;
  orientation::LayoutOrientation=TopToBottom,
  port_size::String="0",
  attrs::AbstractDict=Dict(),
)::Graphviz.Html
...
"""
function ports_horizontal_html_label(
  kind::PortKind,
  nports::Int,
  port_size::String="0",
)::Graphviz.Html
  cols = if nports > 0
    join(
      """<TD HEIGHT="0" WIDTH="$port_size" PORT="$(port_name(kind,i))"></TD>"""
      for i in 1:nports
    )
  else
...
"""
function ports_vertical_html_label(
  kind::PortKind,
  nports::Int,
  port_size::String="0",
)::Graphviz.Html
  rows = if nports > 0
    join(
      """<TR><TD HEIGHT="$port_size" WIDTH="0" PORT="$(port_name(kind,i))"></TD></TR>"""
      for i in 1:nports
    )
  else
...
const default_undirected_graphviz_attrs = (
  ...
  edge=Graphviz.Attributes(:fontname => default_graphviz_font, :len => "0.5"),
)
...
function to_graphviz_property_graph(
  ...
  graph_name::String="G",
  prog::String="neato",
  box_labels::Union{Bool,Symbol}=false,
  port_labels::Bool=false,
  junction_labels::Union{Bool,Symbol}=false,
  junction_size::String="0.075",
  implicit_junctions::Bool=false,
  graph_attrs::AbstractDict=Dict(),
  node_attrs::AbstractDict=Dict(),
  edge_attrs::AbstractDict=Dict(),
)::SymmetricPropertyGraph
...
function html_attributes(attrs::AbstractDict)::String
  join(
    [
      "$(uppercase(string(k)))=\"$v\"" for
      (k, v) in Graphviz.as_attributes(attrs)
    ],
    " ",
  )
end
