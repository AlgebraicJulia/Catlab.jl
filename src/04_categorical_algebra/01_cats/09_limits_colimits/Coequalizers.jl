...
using ..Colimits: AbsColimit, ColimitCocone
import ..Colimits: colimit
using ..Coproducts: ThCategoryUnbiasedCoproducts
import ..Equalizers: factorize

@theory ThCategoryWithCoequalizers <: ThCategoryUnbiasedCoproducts begin
...
coequalizer(C::CatWithCoequalizers, xs...) = coequalizer[getvalue(C)](collect(xs))

coequalizer(m::WithModel, d::AbstractVector; context=nothing) = colimit(m, ParallelMorphisms(d); context)
