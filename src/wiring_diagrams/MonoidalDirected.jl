...
import ..DirectedWiringDiagrams:
  Box, WiringDiagram, input_ports, output_ports, value
...
function permute(A::Ports, σ::Vector{Int}; inverse::Bool=false)
  ...
  if inverse
    ...
    add_wires!(
      f,
      ((input_id(f), σ[i]) => (output_id(f), i) for i in eachindex(σ)),
    )
  else
    f = WiringDiagram(A, B)
    add_wires!(
      f,
      ((input_id(f), i) => (output_id(f), σ[i]) for i in eachindex(σ)),
    )
  end
  return f
end
...
function contravariant_functor(
  d::WiringDiagram,
  f_ports,
  f_box;
  ...
  nports =
    (port::Port) ->
      length((port.kind == InputPort ? input_ports : output_ports)(d, port.box))
  map_port =
    (port::Port) -> Port(
      if (port.box == input_id(d))
        output_id(d)
      elseif (port.box == output_id(d))
        input_id(d)
      else
        port.box
      end,
...
function implicit_mcopy(A::Ports, n::Int)
  ...
  add_wires!(
    f,
    (
      (input_id(f), i) => (output_id(f), i + m * (j - 1)) for i in 1:m for
      j in 1:n
    ),
  )
  return f
...
function implicit_mmerge(A::Ports, n::Int)
  ...
  add_wires!(
    f,
    (
      (input_id(f), i + m * (j - 1)) => (output_id(f), i) for i in 1:m for
      j in 1:n
    ),
  )
  return f
...
mmerge(A::Ports{MonoidalCategoryWithCodiagonals}, n::Int) =
  implicit_mmerge(A, n)
...
mcopy(A::Ports{MonoidalCategoryWithBidiagonals}, n::Int) =
  junctioned_mcopy(A, n)
mmerge(A::Ports{MonoidalCategoryWithBidiagonals}, n::Int) =
  junctioned_mmerge(A, n)
...
Base.:(==)(j1::Junction, j2::Junction) =
  head(j1) == head(j2) &&
  j1.value == j2.value &&
  input_ports(j1) == input_ports(j2) &&
  output_ports(j1) == output_ports(j2)
...
function junction_diagram(make_junction, A::Ports, nin::Int, nout::Int)
  ...
  for (i, value) in enumerate(A)
    v = add_box!(
      f,
      make_junction(value, repeat([value], nin), repeat([value], nout)),
    )
  ...
  for (port, port_value) in enumerate(input_ports(d, v))
    ...
    if nwires != 1
      ...
      add_wires!(
        d,
        [
          source => Port(jv, InputPort, i) for (i, source) in enumerate(sources)
        ],
      )
    end
  end
  ...
  for (port, port_value) in enumerate(output_ports(d, v))
    ...
    if nwires != 1
      ...
      add_wires!(
        d,
        [
          Port(jv, OutputPort, i) => target for
          (i, target) in enumerate(targets)
        ],
      )
    end
  end
...
function rem_junctions(d::WiringDiagram; op=nothing)
  ...
  junction_diagrams = map(junction_ids) do v
    ...
    add_wires!(
      wiring,
      (input_id(wiring), src) => (output_id(wiring), tgt) for
      src in eachindex(inputs), tgt in eachindex(outputs)
    )
    wiring
  end
...
function merge_junctions(d::WiringDiagram; op=nothing)
  ...
  for wire in wires(d, :Wire)
    ...
    if (
      d.diagram[src, :box_type] <: Junction &&
...
dagger(junction::Junction{Op}) where {Op} =
  Junction{Op}(junction.value, output_ports(junction), input_ports(junction))
