...
export read_graphml,
  ...
  write_graphml,
  generate_graphml,
  convert_from_graphml_data,
  convert_to_graphml_data
...
function generate_graphml_ports(
  state::WriteState,
  xnode::XMLElement,
  box::AbstractBox,
)
...
function generate_graphml_data(
  state::WriteState,
  xelem::XMLElement,
  scope::String,
  value,
...
function parse_graphml_node(state::ReadState, xnode::XMLElement)
  ...
  value =
    isempty(data) ? nothing : convert_from_graph_data(state.BoxValue, data)
...
function parse_graphml_ports(state::ReadState, xnode::XMLElement)
  ...
  for xport in xports
    ...
    elseif port_kind == "output"
      ...
      ports[(xnode_id, xport_name)] =
        (kind=OutputPort, port=length(output_ports))
    else
...
function parse_graphml_data(keys::AbstractDict, xelem::XMLElement)
  ...
  for xdata in xelem["data"]
    ...
    key = keys[xkey]
    data[key.attr_name] =
      parse_graphml_data_value(Val{Symbol(key.attr_type)}, xdata)
  end
  data
end
...
parse_graphml_data_value(::Type{Val{:boolean}}, s::String) =
  parse(Bool, lowercase(s))
...
function parse_graphml_metagraph(
  xdoc::XMLDocument;
  directed::Bool=false,
  ...
  keys = parse_graphml_keys(xroot)
  parse_props =
    xnode::XMLElement -> Dict{Symbol,Any}(
