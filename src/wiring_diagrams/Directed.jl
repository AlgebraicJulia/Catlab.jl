...
const WiringDiagramACSet = ACSetType(
  TheoryAttributedWiringDiagram,
  index=[
    :src,
    :tgt,
    :in_src,
...
mutable struct WiringDiagram{Theory,PortValue,WireValue,BoxValue} <: AbstractBox
  diagram::WiringDiagramACSet{
    PortValue,
    WireValue,
    Union{BoxValue,AbstractBox},
    DataType,
  }
  value::Any
  ...
  function WiringDiagram{T,PortValue,WireValue,BoxValue}(
    ...
    inputs::Vector,
    outputs::Vector,
  ) where {T,PortValue,WireValue,BoxValue}
    diagram = WiringDiagramACSet{
      PortValue,
      WireValue,
      Union{BoxValue,AbstractBox},
    ...
    add_parts!(
      ...
      :OuterOutPort,
      length(outputs),
      outer_out_port_type=outputs,
    )
    return f
  end
  function WiringDiagram(
    f::WiringDiagram{T,PortValue,WireValue,BoxValue},
  ) where {T,PortValue,WireValue,BoxValue}
...
function Base.:(==)(d1::WiringDiagram, d2::WiringDiagram)
  (
    ...
    output_ports(d1) == output_ports(d2) &&
    d1.value == d2.value &&
    boxes(d1) == boxes(d2) &&
    sort!(wires(d1)) == sort!(wires(d2))
  )
end
...
function Base.show(io::IO, diagram::WiringDiagram{T}) where {T}
  ...
  if get(io, :compact, false)
    ...
    join(
      io,
      ["$v => $(sshowcompact(box(diagram, v)))" for v in box_ids(diagram)],
      ",\n  ",
    )
    print(io, " ],\n[ ")
...
const WiringDiagramGraph =
  ACSetType(TheoryWiringDiagramGraph, index=[:src, :tgt], unique_index=[:box]){
    Int,
  }
...
in_tgt_box(f::WiringDiagram, w::Int) =
  subpart(f.diagram, w, [:in_tgt, :in_port_box])
out_src_box(f::WiringDiagram, w::Int) =
  subpart(f.diagram, w, [:out_src, :out_port_box])
...
in_port_id(f::WiringDiagram, b::Int, p::Int) =
  incident(f.diagram, b, :in_port_box)[p]
out_port_id(f::WiringDiagram, b::Int, p::Int) =
  incident(f.diagram, b, :out_port_box)[p]
...
function Wire(f::WiringDiagram, ::Type{Val{:PassWire}}, w::Int)
  Wire(
    subpart(f.diagram, w, :pass_wire_value),
...
function Wire(f::WiringDiagram, ::Type{Val{:InWire}}, w::Int)
  Wire(
    subpart(f.diagram, w, :in_wire_value),
...
function Wire(f::WiringDiagram, ::Type{Val{:OutWire}}, w::Int)
  Wire(
    subpart(f.diagram, w, :out_wire_value),
    (
      out_src_box(f, w),
...
function Wire(f::WiringDiagram, ::Type{Val{:Wire}}, w::Int)
  Wire(
    subpart(f.diagram, w, :wire_value),
    (
      src_box(f, w),
...
function wires(f::WiringDiagram, src::Int, tgt::Int)
  ...
  elseif src == input_id(f)
    [
      Wire(f, :InWire, w) for
      w in incident(f.diagram, tgt, [:in_tgt, :in_port_box])
    ]
  elseif tgt == output_id(f)
    [
      Wire(f, :OutWire, w) for
      w in incident(f.diagram, src, [:out_src, :out_port_box])
    ]
  else
    [
      Wire(f, :Wire, w) for
      w in incident(f.diagram, src, [:src, :out_port_box]) if
      tgt_box(f, w) == tgt
    ]
  end
end
...
add_input_ports!(f::WiringDiagram, input_ports::AbstractVector) = add_parts!(
  f.diagram,
  :OuterInPort,
  length(input_ports),
  outer_in_port_type=input_ports,
)
add_output_ports!(f::WiringDiagram, output_ports::AbstractVector) = add_parts!(
  f.diagram,
  :OuterOutPort,
  length(output_ports),
...
function input_ports(f::WiringDiagram, b::Int)
  ...
  elseif b == output_id(f)
    ...
    ) # Copy for backwards compatibilty.
  end
end
...
function add_box!(f::WiringDiagram, box::B) where {B<:AbstractBox}
  b = add_part!(
    ...
    :Box,
    box_type=B,
    value=B <: WiringDiagram ? box : value(box),
  )
  add_parts!(
    f.diagram,
    :InPort,
    length(input_ports(box)),
    in_port_box=b,
    in_port_type=input_ports(box),
  )
  add_parts!(
    f.diagram,
    :OutPort,
    length(output_ports(box)),
    out_port_box=b,
    out_port_type=output_ports(box),
  )
  return b
end
...
function rem_boxes!(f::WiringDiagram, bs::AbstractVector{Int})
  ...
  rem_parts!(
    f.diagram,
    :InWire,
    incident_reduce(f.diagram, bs, [:in_tgt, :in_port_box]),
  )
  rem_parts!(
    f.diagram,
    :OutWire,
    incident_reduce(f.diagram, bs, [:out_src, :out_port_box]),
  )
  rem_parts!(
    f.diagram,
    :Wire,
    incident_reduce(f.diagram, bs, [:tgt, :in_port_box]),
  )
  rem_parts!(
    f.diagram,
    :Wire,
    incident_reduce(f.diagram, bs, [:src, :out_port_box]),
  )
  ...
  rem_parts!(
    f.diagram,
    :OutPort,
    incident_reduce(f.diagram, bs, [:out_port_box]),
  )
...
function rem_wires!(f::WiringDiagram, src::Int, tgt::Int)
  ...
  elseif src == input_id(f)
    rem_parts!(
      f.diagram,
      :InWire,
      incident(f.diagram, tgt, [:in_tgt, :in_port_box]),
    )
  elseif tgt == output_id(f)
    rem_parts!(
      f.diagram,
      :OutWire,
      incident(f.diagram, src, [:out_src, :out_port_box]),
    )
  else
    wires = filter(
      w -> tgt_box(f, w) == tgt,
...
function outneighbors(f::WiringDiagram, b::Int)
  if b == input_id(f)
    vcat(
      subpart(f.diagram, [:in_tgt, :in_port_box]),
...
function inneighbors(f::WiringDiagram, b::Int)
  ...
  elseif b == output_id(f)
    vcat(
      fill(input_id(f), nwires(f, :PassWire)),
...
function in_wires(f::WiringDiagram, b::Int)
  ...
  elseif b == output_id(f)
    ...
    vcat(
      [Wire(f, :Wire, w) for w in incident(f.diagram, b, [:tgt, :in_port_box])],
      [
        Wire(f, :InWire, w) for
        w in incident(f.diagram, b, [:in_tgt, :in_port_box])
      ],
    )
  end
end
...
function out_wires(f::WiringDiagram, b::Int)
  ...
  elseif b == output_id(f)
    ...
    vcat(
      [
        Wire(f, :Wire, w) for w in incident(f.diagram, b, [:src, :out_port_box])
      ],
      [
        Wire(f, :OutWire, w) for
        w in incident(f.diagram, b, [:out_src, :out_port_box])
      ],
    )
  end
end
...
function substitute(
  ...
  vmap =
    Dict(input_id(d) => input_id(result), output_id(d) => output_id(result))
...
function encapsulate(
  d::WD,
  vss::Vector{Vector{Int}};
  discard_boxes::Bool=false,
  make_box=Box,
  values=nothing,
) where {WD<:WiringDiagram}
  if isempty(vss)
  ...
  encapsulated_representatives =
    Dict(minimum(vs) => (vs, value) for (vs, value) in zip(vss, values))
  ...
  for v in box_ids(d)
    if haskey(encapsulated_representatives, v)
      ...
      sub, sub_map = encapsulated_subdiagram(
        d,
        vs;
        discard_boxes=discard_boxes,
        make_box=make_box,
        value=value,
      )
      ...
      merge!(
        port_map,
        Dict(port => Port(subv, data...) for (port, data) in sub_map),
      )
  ...
  for wire in wires(d)
    src, tgt = wire.source, wire.target
    new_src = if haskey(vmap, src.box)
      set_box(src, vmap[src.box])
    elseif haskey(port_map, src)
      port_map[src]
    else
      continue
    end
    new_tgt = if haskey(vmap, tgt.box)
      set_box(tgt, vmap[tgt.box])
    elseif haskey(port_map, tgt)
      port_map[tgt]
    else
      continue
    end
...
"""
function encapsulated_subdiagram(
  d::WD,
  vs::Vector{Int};
  discard_boxes::Bool=false,
  make_box=Box,
  ...
  for v in vs
    ...
    for (port, value) in enumerate(input_ports(d, v))
      ...
      srcs = sort!([
        wire.source for
        wire in in_wires(d, tgt) if !haskey(vmap, wire.source.box)
      ])
      if isempty(srcs)
        continue
      end
      src = get!(inner_port_map, (srcs, value)) do
        p =
          discard_boxes ? length(push!(inputs, value)) :
          add_outer_in_port!(result, value)
    ...
    for (port, value) in enumerate(output_ports(d, v))
      ...
      tgts = sort([
        wire.target for
        wire in out_wires(d, src) if !haskey(vmap, wire.target.box)
      ])
      if isempty(tgts)
        continue
      end
      tgt = get!(inner_port_map, (tgts, value)) do
        p =
          discard_boxes ? length(push!(outputs, value)) :
          add_outer_out_port!(result, value)
