...
using ...Graphs,
  ..DirectedWiringDiagrams,
  ..UndirectedWiringDiagrams,
  ..MonoidalDirectedWiringDiagrams,
  ..MonoidalUndirectedWiringDiagrams
...
function to_undirected_wiring_diagram(
  expr::GATExpr,
  Ob::Type,
  Hom::Type,
  ob_map,
    ...
    terms=Dict(
      ...
      :Hom =>
        expr -> begin
          ...
          HDHom(
            singleton_diagram(A ⊗ B, name=hom_map(expr)),
...
function to_hom_expr(Ob::Type, Hom::Type, d::WiringDiagram)
  ...
  if nboxes(d) > 1
    @assert all(
      (wire.source.box == input_id(d) || wire.target.box == output_id(d)) for
      wire in wires(d)
    )
    d = encapsulate_parallel(
      Ob,
      Hom,
      d,
...
function parallel_reduction(Ob::Type, Hom::Type, d::WiringDiagram)
  ...
  parallel = Dict([
    ((box_id(g, src), box_id(g, tgt)), box_id(g, vs)) for
    ((src, tgt), vs) in parallel
  ])
  encapsulate_parallel(
    Ob,
    Hom,
    d,
...
function input_parallel_reduction(Ob::Type, Hom::Type, d::WiringDiagram)
  ...
  encapsulate_parallel(
    ...
    Hom,
    d,
    [(box_id(g, parallel[src]), [box_id(g, src)], Int[])],
  )
end
...
function output_parallel_reduction(Ob::Type, Hom::Type, d::WiringDiagram)
  ...
  encapsulate_parallel(
    ...
    Hom,
    d,
    [(box_id(g, parallel[tgt]), Int[], [box_id(g, tgt)])],
  )
end

function encapsulate_parallel(Ob::Type, Hom::Type, d::WiringDiagram, parallel)
  parallel = Vector{Int}[
    vs[crossing_minimization_by_sort(d, vs, sources=sources, targets=targets)] for (vs, sources, targets) in parallel
  ]
...
function junction_to_expr(Ob::Type, junction::Junction)
  ...
  elseif (nin == 0 && nout == 2)
    ...
    compose_simplify_id(mmerge_foldl(ob, nin), mcopy_foldl(ob, nout))
  end
end
...
function find_series(
  ...
  for edge in edges(g)
    s, t = src(g, edge), tgt(g, edge)
    if (
      length(unique(outneighbors(g, s))) == 1 &&
