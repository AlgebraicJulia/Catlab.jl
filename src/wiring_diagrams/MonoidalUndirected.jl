...
export HypergraphDiagram,
  HypergraphDiagramOb,
  HypergraphDiagramHom,
  ObUWD,
  HomUWD,
  cospan_action,
  dom_mask,
...
const UntypedHypergraphDiagram = ACSetType(
  TheoryUntypedHypergraphDiagram,
...
const HypergraphDiagram =
  ACSetType(TheoryHypergraphDiagram, index=[:box, :junction, :outer_junction])
...
@auto_hash_equals struct HomUWD{UWD<:AbstractUWD}
  ...
  function HomUWD{UWD}(
    ...
    codom::Union{BitVector,Nothing}=nothing,
  ) where {UWD<:AbstractUWD}
...
@instance HypergraphCategory{ObUWD,HomUWD} begin
  ...
  function compose(f::HomUWD, g::HomUWD)
    ...
    HomUWD(
      compose(f.diagram, g.diagram, codom_mask(f), dom_mask(g)),
      dom=(1:(k + ℓ) .<= k),
    )
  end
  ...
  braid(A::ObUWD, B::ObUWD) =
    let m = length(A), n = length(B)
      HomUWD(
        junction_diagram(A ⊗ B, [1:(m + n); (m + 1):(m + n); 1:m]),
