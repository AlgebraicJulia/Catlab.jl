...
with identity domain to the given proarrow that is univeral among all cells of 
...
@theory ThDoubleCategoryWithTabulators{Ob,Hom,Pro,Cell,Tab} <: ThDoubleCategory{Ob,Hom,Pro,Cell} begin
  ...
  (universal(τ,θ) ⋅ proj1(τ)) == f ⊣ (A::Ob, B::Ob, X::Ob, p::(A↛B), 
      f::(X→A), g::(X→B), τ::Tab(p), θ::Cell(pid(X),p,f,g))
  (universal(τ,θ) ⋅ proj2(τ)) == g ⊣ (A::Ob, B::Ob, X::Ob, p::(A↛B), 
      f::(X→A), g::(X→B), τ::Tab(p), θ::Cell(pid(X),p,f,g))
  (pid(universal(τ,θ)) ⋅ cell(τ)) == θ ⊣ (A::Ob, B::Ob, X::Ob, 
      p::(A↛B), f::(X→A), g::(X→B), τ::Tab(p), θ::Cell(pid(X),p,f,g))
