export ThCategory, FreeCategory, Ob, Hom, dom, codom, id, compose, ⋅,
  ...
  ThPtCategory, FreePtCategory, ZeroHom, ZeroHomExpr, Z,z
...
and a distinguished weak zero object. That is, we don't axiomatize 
that morphisms to and from the zero object `Z` are unique,
although they will be in an instance freely generated by a category.
...
of partially-defined functor into the complement of the
zero maps and zero object in the codomain.
"""
@theory ThPtCategory{Ob,Hom,ZeroHom} <: ThCategory{Ob,Hom} begin
  Z()::Ob
  ZeroHom(dom::Ob,codom::Ob)::TYPE
  z(A::Ob,B::Ob)::ZeroHom(A,B)
  hom(x::ZeroHom(A,B))::Hom(A,B)⊣(A::Ob,B::Ob)
  compose(x::ZeroHom(A,B),f::(B→C))::ZeroHom(A,C)⊣(A::Ob,B::Ob,C::Ob)
  compose(g::(A→B),x::ZeroHom(B,C))::ZeroHom(A,C)⊣(A::Ob,B::Ob,C::Ob)
  z₁==z₂⊣(A::Ob,B::Ob,z₁::ZeroHom(A,B),z₂::ZeroHom(A,B))
end

abstract type ZeroHomExpr{T} <: CategoryExpr{T} end
@syntax FreePtCategory{ObExpr,HomExpr,ZeroHomExpr} ThPtCategory begin
  compose(f::Hom,g::Hom) = associate_unit(new(f,g; strict=true), id)
  #no strictness imposed thus far
