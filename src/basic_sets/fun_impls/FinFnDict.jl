...
@instance ThFinDomFunction{D,C} [model::FinFunctionDict{SetOb, D,C}] where {D,C} begin
  ...
  function postcompose(g::SetFunction)::FinDomFunction′
    FinDomFunction(Dict(k => g(v) for (k,v) in getvalue(model)), codom(g))
  end
end
...
""" Default `FinFunction` from a `AbstractDict`"""
FinDomFunction(f::AbstractDict) = FinDomFunction(f, FinSet(Set(values(f))))

""" Default `FinDomFunction` from a `AbstractDict` and codom"""
FinDomFunction(f::AbstractDict, cod::SetOb) = 
  FinDomFunction(FinDomFunctionDict(f, cod))
