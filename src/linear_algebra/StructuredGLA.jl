...
export StructuredLinearFunctions,
  FreeStructuredLinearFunctions,
  Ob,
  Hom,
  dom,
  codom,
...
@theory StructuredLinearFunctions{Ob,Hom} <: LinearFunctions{Ob,Hom} begin
  ...
  tridiag(v, u, w)::(A ⊕ ℝ() → A ⊕ ℝ()) ⊣
  (A::Ob, v::(ℝ() → A ⊕ ℝ()), u::(ℝ() → A), w::(ℝ() → A))
  symtridiag(v, u)::(A ⊕ ℝ() → A ⊕ ℝ()) ⊣
  (A::Ob, v::(ℝ() → A ⊕ ℝ()), u::(ℝ() → A))
  ...
  (A::Ob, a::(ℝ() → A ⊗ ℝ()), b::(ℝ() → A))
  tridiag(a, b, c) ==
  diag(a) + lowerdiag(b) + upperdiag(c) ⊣
  (A::Ob, a::(ℝ() → A ⊗ ℝ()), b::(ℝ() → A), c::(ℝ() → A))
  symtridiag(a, b) ==
  tridiagonal(a, b, b) ⊣
  (A::Ob, a::(ℝ() → A ⊗ ℝ()), b::(ℝ() → A))
end
