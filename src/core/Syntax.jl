...
macro syntax(syntax_head, mod_name, body=nothing)
  ...
  expr = Expr(
    :call,
    :syntax_code,
    Expr(:quote, syntax_name),
    esc(Expr(:ref, :Type, base_types...)),
    esc(mod_name),
    esc(nameof(__module__)),
    functions,
  )
  Expr(
    :block,
...
function interface(theory::Theory)::Vector{JuliaFunction}
  [
    ...
    [
      GAT.constructor(constructor_for_generator(cons), theory) for
      cons in theory.types
    ]
  ]
end
...
"""
function gen_term_constructor(
  cons::TermConstructor,
  theory::Theory,
  mod::Module;
  dispatch_type::Symbol=Symbol(),
)::Expr
  ...
  if !isempty(eqs)
    ...
    insert!(call_expr.args, 2, Expr(:parameters, Expr(:kw, :strict, false)))
    push!(
      body.args,
      Expr(
        :if,
        ...
        Expr(
          ...
          Expr(
            :call,
            GlobalRef(Syntax, :SyntaxDomainError),
...
function replace_nullary_constructors(expr, theory)
  @match expr begin
    ...
    Expr(:call, name::Symbol, args...) => Expr(
      :call,
      name,
      [replace_nullary_constructors(a, theory) for a in args]...,
    )
    _ => expr
  end
end
...
"""
function gen_term_generator(
  cons::TypeConstructor,
  theory::Theory,
  mod::Module,
)::Expr
  gen_term_constructor(
    constructor_for_generator(cons),
    theory,
    mod;
...
function invoke_term(syntax_module::Module, constructor_name::Symbol, args...)
  ...
  theory = GAT.theory(theory_type)
  syntax_types =
    Tuple(getfield(syntax_module, cons.name) for cons in theory.types)
    ...
    args = map(enumerate(sexpr[2:end])) do (i, arg)
      arg_kind =
        (
          ...
          arg isa Union{Bool,Number,Nothing}
        ) ? :value : :expr
