...
abstract type PathCatImpl{Ob,Gen} <: Model{Tuple{Ob,Gen,FinSet,Vector{<:Pair}}} end
...
"""
@struct_hash_equal struct PathCat{Ob,Gen} <: FinCatImpl{Ob,Path{Ob,Gen},Gen}
...
equations(p::PathCat) = equations(getvalue(p))
