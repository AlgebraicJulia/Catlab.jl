...
"""
function graph(C::FinCat;maps::Bool=false,inv::Bool=false,named::Bool=false) 
  g =  DiagramGraph() 
  ...
  named || begin g′ = Graph(); copy_parts!(g′,g,(:V,:E)); g = g′ end
  maps || return g
...
"""
The opposite of a FinCatPresentation whose syntax is not just FreeCategory
is effectively interpreted as if it were, since the opposite may not 
strictly speaking exist in another syntax.
"""
function hom_generators(C::OppositeCat{Ob,Hom,FinCatSize,Ct}) where {Ob,Hom,Ct<:FinCatPresentation}
  S = presentation(C).syntax
  map(x->S.Hom(nameof(x),S.Ob(S.Ob,nameof(codom(x))),S.Ob(S.Ob,nameof(dom(x)))),hom_generators(C.cat))
end
"""
...
"""
function dom_to_graph(F::FinDomFunctor{<:FinCatPresentation,<:Cat{Ob,Hom}};named::Bool=false) where {Ob,Hom}
  g, obs, homs = graph(dom(F),maps=true,named=named)
  C = FinCat(g)
