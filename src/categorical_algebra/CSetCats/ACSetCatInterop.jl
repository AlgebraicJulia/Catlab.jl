module ACSetCatInterop 

using StructEquality
using ACSets
using ....Theories
using ...Cats.Categories, ...Cats.FinCats# , ...Cats.Diagrams
import ...Cats.FinFunctors: FinDomFunctor
using ...Cats.Categories: FunctorImpl
...
const ACSetDomCat = FinCats.FinCatPresentation{FreeSchema}#{
...
"""
@struct_hash_equal struct ACSetFunctor{ACS<:ACSet, Ob, Hom, C<:Cat{Ob,Hom}
    } <: FunctorImpl{GATExpr{:generator}, Ob, GATExpr, Hom, ACSetDomCat, C}
  acset::ACS
  cod::C
end
...
function (::Type{ACS})(F::T) where {ACS <: ACSet, T<:FinDomFunctor}
