...
export ACSetMorphism, 
  ...
  StructTightACSetTransformation, components, type_components, force,
  naturality_failures, show_naturality_failures, is_natural, in_bounds, is_cartesian
...
using ...Cats.FreeDiagrams, ...Cats.Limits, ...Cats.Subobjects
using ...BasicSets.Sets, ...BasicSets.SetFunctions
using ...BasicSets.FinSets, ...BasicSets.FinFunctions
using ...BasicSets.FinFunctions: IdentityFunction
import ...Cats.Limits: limit, colimit, universal
import ...Cats.Subobjects: Subobject, implies, ⟹, subtract, \, negate, ¬, non, ~
# using ...Cats.Diagrams: Diagram, diagram TODO
import ...BasicSets.FinFunctions: force, is_monic, is_epic, preimage
import ...Cats.FinFunctors: FinDomFunctor, is_natural, components
...
@struct_hash_equal struct StructACSetTransformation{
    ...
    Codom <: StructACSet{S}} <: ACSetTransformation{Dom,Codom}
  components::Comp
  dom::Dom
  codom::Codom  
...
"""
@struct_hash_equal struct DynamicACSetTransformation <:
    ACSetTransformation{ACSet,ACSet}
...
@instance ThCategory{ACSet, ACSetTransformation} [model::ACSetTight] begin
  @import Ob, Hom, →
...
@cocartesian_monoidal_instance ACSet ACSetTransformation ACSetTight

@default_model ThCategory{ACSet, ACSetTransformation} [model::ACSetTight]
