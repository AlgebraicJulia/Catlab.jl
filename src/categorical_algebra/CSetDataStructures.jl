...
"""
abstract type AbstractAttributedCSet{CD<:CatDesc,AD<:AttrDesc{CD},Ts<:Tuple} end
...
) where {CD<:CatDesc,AD<:AttrDesc{CD},Ts<:Tuple,Idxed,UniqueIdxed}
...
function ACSetType(pres::Presentation{Schema}; index=[], unique_index=[])
  ...
  T = ACSet{
    SchemaType(pres)...,
    Tuple{type_vars...},
    Tuple(sort!(index ∪ unique_index)),
    Tuple(sort!(unique_index)),
  }
  foldr(UnionAll, type_vars, init=T)
end
...
@generated function ACSetTableDataType(
  ::Type{X},
  ::Type{Val{ob₀}},
) where {CD<:CatDesc,AD<:AttrDesc{CD},Ts,X<:AbstractACSet{CD,AD,Ts},ob₀}
  CD₀, AD₀ = ACSetTableDesc(CD, AD, ob₀)
  :(ACSet{$CD₀,$AD₀,$Ts,(),()})
end

@generated function ACSetTableUnionAll(
  ::Type{X},
  ::Type{Val{ob₀}},
) where {CD<:CatDesc,AD<:AttrDesc{CD},X<:AbstractACSet{CD,AD},ob₀}
  CD₀, AD₀ = ACSetTableDesc(CD, AD, ob₀)
  :(ACSet{$CD₀,$AD₀,Tuple{$(data(AD)...)},(),()} where {$(data(AD)...)})
end

function ACSetTableDesc(
  ::Type{CD},
  ::Type{AD},
  ob₀::Symbol,
) where {CD<:CatDesc,AD<:AttrDesc{CD}}
  @assert ob₀ ∈ ob(CD)
...
function CSetType(pres::Presentation{Schema}; index=[], unique_index=[])
  if !(isempty(generators(pres, :Data)) && isempty(generators(pres, :Attr)))
    error(
      "Use `ACSetType` instead of `CSetType` for schemas with data attributes",
    )
  end
  CSet{
    CatDescType(pres),
...
function make_tables(
  ...
  for attr in attr(AD)
    ...
    push!(col.names, attr)
    push!(col.types, Ts.parameters[codom_num(AD, attr)])
  end
  map(cols) do col
...
function Base.show(io::IO, acs::T) where {CD,AD,Ts,T<:AbstractACSet{CD,AD,Ts}}
  ...
  join(
    ...
    vcat(
      ...
      [
        "  $hom : $(dom(CD,i)) → $(codom(CD,i)) = $(subpart(acs,hom))" for
        (i, hom) in enumerate(hom(CD))
      ],
      [
        "  $attr : $(dom(AD,i)) → $(codom(AD,i)) = $(subpart(acs,attr))" for
        (i, attr) in enumerate(attr(AD))
      ],
    ),
    ",\n",
  )
...
@generated function _has_subpart(
  ::ACSet{CD,AD},
  ::Type{Val{name}},
) where {CD,AD,name}
...
"""
@inline subpart(acs::ACSet, part, name) =
  view_or_slice(subpart(acs, name), part)
...
@generated function _incident(
  acs::ACSet{CD,AD,Ts,Idxed},
  part,
  ::Type{Val{name}};
  copy::Bool=false,
) where {CD,AD,Ts,Idxed,name}
  if name ∈ hom(CD)
    if name ∈ Idxed
      quote
  ...
  quote
    ...
    $(Expr(:block, map(indexed_in_homs) do hom
      quote
        ...
        @inbounds for i in start:nparts
          acs.indices.$hom[i] = Int[]
        end
      end
    end...))
    start:nparts
...
function _set_subpart!(
  acs::ACSet,
  part::AbstractVector{Int},
  ::Type{Name},
  subpart,
) where {Name}
...
@generated function set_subparts!(
  acs::ACSet,
  part,
  subparts::NamedTuple{names},
) where {names}
  Expr(
    :block,
    map(names) do name
...
@generated function _copy_parts!(
  to::ACSet{CD},
  from::ACSet{CD′},
  ...
  quote
    ...
    partmaps = NamedTuple{$(Tuple(in_obs))}(
      tuple($(map(in_obs) do type
...
"""
@generated function copy_parts_only!(
  to::ACSet{CD},
  from::ACSet{CD′};
  kw...,
) where {CD,CD′}
        ...
        $(
          map(obs) do ob
            Expr(
              :kw,
              ob,
              :(add_parts!(to, $(QuoteNode(ob)), length(parts.$ob))),
            )
          end...
        ),
      )
...
function set_data_index!(
  d::AbstractDict{K,<:AbstractVector{Int}},
  k::K,
  v::Int,
) where {K}
  insertsorted!(get!(d, k) do
    Int[]
  end, v)
end
...
"""
function init_acset(
  T::Type{<:ACSet{CD,AD,Ts}},
  body,
) where {CD<:CatDesc,AD<:AttrDesc{CD},Ts<:Tuple}
...
@generated function _map(
  acs::AT,
  fns::NamedTuple{map_over},
) where {map_over,CD,AD,Ts,Idxed,UniqIdxed,AT<:ACSet{CD,AD,Ts,Idxed,UniqIdxed}}
  map_over = [map_over...]
  ...
  all_attrs == needed_attrs ||
    error("not enough functions provided to fully transform ACSet")
