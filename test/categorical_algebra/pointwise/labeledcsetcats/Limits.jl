...
A = @acset WG{Float64} begin V=2; E=3; 
  src=[1, 2, 1]; tgt=[1, 2, 2]; weight=[-1, -1, 5] 
end
B = @acset WG{Float64} begin V=3; E=5; 
...
C = @acset WG{Float64} begin V=1; E=2; Weight=2
...
ABC = pullback(AC,BC);
expected = @acset WG{Float64} begin V=6; E=7; Weight=3; 
...
expected = @acset WG{Float64} begin V=6; E=7;
