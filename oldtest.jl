...
# Two function α⇉β and their equalizer f×ᵦg
G = Graph(3);
...
PairG = Graph(2); # s s²
add_edges!(PairG, [2, 2], [1, 1]);

Span2 = Graph(3)
add_edges!(Span2, [3,3],[1,2]);

PairSketch = FLS(PairG, T[], T[
    T(Span2, PairG, V=[1,1,2],  E=[1, 2])]);
srch = cs -> search(PairSketch, cs, [:s, :s²], [:π₁,:π₂]);
pair_res = search(PairSketch, [2,4])[1];

CPG = Graph(3); # s s² s³
add_edges!(CPG, [2, 2, 3, 3, 3],
                [1, 1, 1, 1, 1]);
...
CpSketch = FLS(CPG, T[], T[
    T(Span2, CPG, V=[1,1,2],  E=[1, 2]),
    T(Span3, CPG, V=[1,1,1,3],E=[3,4,5])]);
srch = cs -> search(CpSketch, cs, [:s, :s², :s³],
                    [:π₁,:π₂,:Π₁,:Π₂,:Π₃]);
ig = init_grph(CPG, [2,4,8]);
c1 = choose(ig,2,1,1)[1];
c2 = choose(c1, 2,2,4)[1];
c3 = choose(c2, 2,3,3)[1];
c4 = choose(c3, 2,4,4)[1];

trip_res = search(CpSketch, [2,4,8], -1, c4); # found on the order of seconds

# This is too big to solve without first solving pairs.
# takes minutes to even canonical hash the initial graph
# can we add a pruning step to the search algorithm
# based on coloring of the model space?
# there is a lot of symmetry
# if all FK choices (pk1 fk1 fk2 fk3 fk4)
#                   (pk1 fk1 fk2 fk3 fk5) ...
# are the same color, can we pick one arbitrarily?
#
...
# For the smallest nontrivial one, the foreign keys on s³ have
# 16384 possibilities.
...
ig = init_grph(SGG, [2,4,8]);
c1 = choose(ig,2,1,1)[1];
c2 = choose(c1, 2,2,4)[1];
c3 = choose(c2, 2,3,4)[1];
c4 = choose(c3, 2,4,4)[1];

# Need to restrict all of s³ with pk1
# to those that have e4=e5=1, etc.
